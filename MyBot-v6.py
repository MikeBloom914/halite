...
game = hlt.Game("SheckyBotbeatjtv6 half times")
...
### can do distance to coordinates for diff coordinance ###
        ...
        if turn_num <= 20:
            ...
            elif len(my_planets_not_full) == len(my_planets):
            ...
            if shipid % 2 == 0 and len(closest_empty_planets) > 0:
                target_planet = closest_empty_planets[0]
                if target_planet:
                    if ship.can_dock(target_planet):
                        command_queue.append(ship.dock(target_planet))
                        logging.info("shipid: " + str(ship.id))
                    else:
                        navigate_command = ship.navigate(
                            ship.closest_point_to(target_planet),
                            game_map,
